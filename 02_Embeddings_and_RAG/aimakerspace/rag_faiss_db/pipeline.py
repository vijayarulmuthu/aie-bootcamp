...
class RAGPipelineFAISSDB:
    def __init__(self, llm: ChatOpenAI, vector_db: FAISSVectorDatabase):
        self.llm = llm
    ...
    def _init_db(self):
        ...
        try:
            ...
            results_dir = "rag_faiss_db/results"
            try:
            ...
            self.cursor.execute('''
                CREATE TABLE IF NOT EXISTS rag_results_faiss_db (
...
"""
    ...
    def _extract_context(self, search_results: List[Tuple[str, float]], min_score: float = 0.7) -> Tuple[str, List[float]]:
        """
...
"""
    ...
    async def run_pipeline(self, query: str, search_method: SearchMethod) -> Dict:
        ...
        try:
            ...
            try:
                context, scores = self._extract_context(search_results)
        ...
        try:
            self.cursor.execute('''
                INSERT INTO rag_results_faiss_db 
...
async def main():
    ...
    indices_dir = "rag_faiss_db/indices"
    try:
        os.makedirs(indices_dir, exist_ok=True)
    except OSError as e:
        raise FilePermissionError(f"Failed to create indices directory: {str(e)}")
    
    # Create databases for each search method
    dbs = {
        SearchMethod.FLAT: FAISSVectorDatabase(
            embedding_model=EmbeddingModel(),
            search_method=SearchMethod.FLAT,
            persist_dir=f"{indices_dir}/flat"
        ),
        SearchMethod.LSH: FAISSVectorDatabase(
            embedding_model=EmbeddingModel(),
            search_method=SearchMethod.LSH,
            persist_dir=f"{indices_dir}/lsh"
        ),
        SearchMethod.HNSW: FAISSVectorDatabase(
            embedding_model=EmbeddingModel(),
            search_method=SearchMethod.HNSW,
            persist_dir=f"{indices_dir}/hnsw"
        ),
        SearchMethod.IVF: FAISSVectorDatabase(
            embedding_model=EmbeddingModel(),
            search_method=SearchMethod.IVF,
            persist_dir=f"{indices_dir}/ivf"
        )
    }
    
    # Upload document to each database
    print("Uploading document to all databases...")
    for method, db in dbs.items():
        print(f"Uploading to {method.value} database...")
        try:
            await db.upload_document("data_files/sample.pdf", chunking="Token", batch_size=50)
        except FileNotFoundError as e:
            ...
            print(f"Error uploading to {method.value} database: {str(e)}")
            continue
    ...
    for query in queries:
        ...
        for method, db in dbs.items():
            ...
            try:
                rag_pipeline = RAGPipelineFAISSDB(llm, db)
